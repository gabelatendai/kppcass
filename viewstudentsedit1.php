...
if (isset($_POST['register'])) {
   ...
   // $department_id = $_POST['department_id'];
    ...
    //$ini = R::findOne('users', 'email = ? AND id = ?', [$email, $user_id]);
   ...
   // $init = R::findOne('users', 'email  = ? ', [$email]);
    ...
    if ($admission_number==""){
        ...
        mysqli_query($conn,"UPDATE students SET `admission_number`='$admission_number',`gender`='$gender',`firstname`='$firstname',`lastname`='$lastname',`idno`='$idno',`dateofbirth`='$dateofbirth',`mobile`='$mobile',`email`='$email',`address`='$address',`zipcode`='$zipcode',
                ,intake='$intake' ,level='$level' 
                WHERE `id`='$id'");

        ?>
        <script>
            alert('Record Succsessfully Updated');
            window.location = "complete_reg";
        </script>
        <?php
 ...
 zipcode='$zipcode ', gender='$gender' ,intake='$intake' ,level='$level' WHERE id = '$id'");
        ...
        */
	    ...
	    <script>
		    alert('Record Succsessfully Updated');
		    window.location = "complete_reg?id=<?php echo $id;?>";
	    </script>
        <?php
    }
